...
class KernelCoreMEB:
    """
        coefficients are alphas
    """
    
    def __init__(self, point_set:[], eps:float, kernel_fun=elm_kernel_vec):
...
#        print(1)
        while delta > (1.0 + self.eps) * (1.0 + self.eps) - 1.0:
            ...
            if delta > delta_minus:
                ...
                lambda1 = delta_minus / (2.0  * (3.0 - delta_minus))
...
#                self.c_norm += (self.coefficents[idx1] * self.coefficents[idx2] * self.kernel_cache[idx1][idx2])
        ...
        coeff_arr = np.array(list(self.coefficents.values()))
        self.c_norm = np.sum(np.outer(coeff_arr, coeff_arr) * np.array(list(self.kernel_cache.values())))
         
    def add_to_kernel_matrix(self, idx:int) -> None:
        kernel_vector = []
        for i in range(len(self.points)):
            kernel_vector.append(self.kernel_eval(self.points[idx], self.points[i]))
    ...
    def _dist2_wc(self, point:[]) -> float:
        # TODO _dist2_wc + idx method, implement it with Eq 12
        dist2 = 0.0
        for idx in self.core_indices:
            dist2 += self.coefficents[idx] * self.kernel_eval(self.core_points[idx], point)
        
        return self.kernel_eval(point, point) - 2.0 * dist2 + self.c_norm # Eq12
#        return 1.0 + self.c_norm - 2.0 * dist2
    ...
    def _dist2_wc_idx(self, idx1:int) -> float:
        ...
        return self.kernel_cache[idx1][idx1] - 2.0 * dist2 + self.c_norm # Eq12
#        return 1.0 + self.c_norm - 2.0 * dist2
    
    def _k_dist2(self, p1:[], p2: []) -> float:
        # TODO is this okay?
        return (2.0 * self.kernel_eval(p1, p1) - 2.0 * self.kernel_eval(p1, p2))
#        return (2.0 - 2.0 * self.kernel_eval(p1, p2))
...
#            delta = 1.0
            ...
            kernel = self.kernel_fun(p1['data'], p2['data'])
            delta = 0.0
            
        label_prod = p1['label'] @ p2['label']

         # TODO: this is a problem because kernel is no longer limited to a max of 1
        return label_prod * kernel + label_prod + delta# (eq 11)
