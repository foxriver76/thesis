...
class AFRSLVQ(ClassifierMixin, BaseEstimator):
    ...
    def _optimize(self, X, y, random_state):
        ...
        if X[0].size > self.w_[0].size:
            diff = X[0].size - self.w_[0].size
            print('NEW DIM DETECTED - DIFF: {}'.format(diff))
            new_w = np.zeros((self.w_[:, 0].size, X[0].size))
            self.squared_mean_gradient = np.zeros((self.w_[:, 0].size, X[0].size))
            self.squared_mean_step = np.zeros((self.w_[:, 0].size, X[0].size))
            for i in range(self.w_[:, 0].size): # iterate over prototypes
                new_w[i] = np.append(self.w_[i], X[0][-diff:])
            self.w_ = new_w
...
#                             "expected=%d" % (self.w_.shape[1], x.shape[1]))
        proto_dim = self.w_[0].size
        """Detect new features - we suggest that new features are appended"""
        if x[0].size > proto_dim:
            x = x[0][0:proto_dim].reshape(x[0, 0].size, proto_dim)
