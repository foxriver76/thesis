...
clf_rslvq = RandomProjectionClassifier(clf=ARSLVQ(), n_components=1000, high_dims=10000)
clf_rrslvq = RandomProjectionClassifier(clf=RRSLVQ(), n_components=1000, high_dims=10000)
clf_arf = RandomProjectionClassifier(clf=ARF(), n_components=1000, high_dims=10000)
clf_samknn = RandomProjectionClassifier(clf=SAMKNN(), n_components=1000, high_dims=10000)

evaluator = EvaluatePrequential(metrics=['model_size', 'accuracy'], 
                                max_samples=N_SAMPLES, show_plot=False,
                                output_file='_rp_mem_w_proj.csv')
