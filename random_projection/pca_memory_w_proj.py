...
clf_rslvq = PCAClassifier(clf=ARSLVQ(), n_components=48, high_dims=250)
clf_rrslvq = PCAClassifier(clf=RRSLVQ(), n_components=48, high_dims=250)
clf_arf = PCAClassifier(clf=ARF(), n_components=48, high_dims=250)
clf_samknn = PCAClassifier(clf=SAMKNN(), n_components=48, high_dims=250)

evaluator = EvaluatePrequential(metrics=['model_size', 'accuracy'], 
                                max_samples=N_SAMPLES, show_plot=False,
                                output_file='_pca_mem_w_proj.csv', 
                                batch_size=50)
